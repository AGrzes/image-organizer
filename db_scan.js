...
module.exports = (db, patterns) => {
  var bases = patterns.map(globParent)
  return miss.pipeline.obj(new Ouch(db).view((doc, emit) => {
    if (_.some(doc.files, (status, file) => bases.some((base) => file.startsWith(base)))) {
      emit(doc._id)
    }
  }, {
    include_docs: true
  }), miss.through.obj((chunk, enc, cb) => cb(null, chunk.doc)), miss.through.obj(function (doc, enc, cb) {
    _(doc.files).pickBy((status, file) => patterns.some((pattern) => minimatch(file, pattern))).forEach((status, file) => {
      this.push({
        doc: doc,
        file: file,
        exif: doc.exif
